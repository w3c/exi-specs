...
<!-- xmlspec.xsl: An HTML XSL[1] Stylesheet for XML Spec V2.1[2] markup
     ...
     Version: $Id: xmlspec.xsl,v 1.11 2018/02/13 14:44:41 carine Exp $
     ...
     Date:    Created 07 September 1999
              Last updated $Date: 2018/02/13 14:44:41 $ by $Author: carine $
...
<!-- ====================================================================== -->
  ...
  <xsl:param name="toc.level" select="6"/>
...
<!-- MJH modified to number examples -->
          <i><span><xsl:text>Example </xsl:text>
  ...
  <xsl:template match="header">
    ...
    <!-- xsl:apply-templates select="notice"/ --><!-- moved after the pubdate. -->
...
<!--
  ...
  <!-- warning: a warning about the spec -->
  ...
  <xsl:template match="spec">
	...
	<meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no" />
        <style>
        <xsl:text>body { background-image: url('https://www.w3.org/StyleSheets/TR/</xsl:text>
        <xsl:choose>
          <xsl:when test="/spec/@role='editors-copy'">logo-MO</xsl:when>
          <xsl:otherwise>
            <xsl:choose>
	      <!-- Editor's review drafts are a special case. -->
              ...
              <!--<xsl:when test="/spec/@w3c-doctype='review'
                ...
                >W3C-MOX</xsl:when>-->
              <!-- Daniel Peintner: extension to support proper Editor's Draft -->
              <xsl:when test="contains(/spec/header/w3c-doctype, 'Editor')"
                >logo-ED</xsl:when>
              <!-- End of extension -->
              <xsl:when test="/spec/@w3c-doctype='wd'">logo-WD</xsl:when>
              <xsl:when test="/spec/@w3c-doctype='rec'">logo-REC</xsl:when>
              <xsl:when test="/spec/@w3c-doctype='pr'">logo-PR</xsl:when>
              <xsl:when test="/spec/@w3c-doctype='per'">logo-PER</xsl:when>
              <xsl:when test="/spec/@w3c-doctype='cr'">logo-CR</xsl:when>
              <xsl:when test="/spec/@w3c-doctype='note'">logo-NOTE</xsl:when>
              <xsl:when test="/spec/@w3c-doctype='wgnote'">logo-WG-NOTE</xsl:when>
              <xsl:when test="/spec/@w3c-doctype='memsub'">logo-Member-SUBM</xsl:when>
              <xsl:when test="/spec/@w3c-doctype='teamsub'">logo-Team-SUBM</xsl:when>
  ...
  <xsl:template match="specref">
	...
	<xsl:message>
      ...
      <xsl:when test="local-name($target) = 'termdef'">
        <xsl:text>"</xsl:text>
        <a>
          <xsl:attribute name="href">
            <xsl:call-template name="href.target">
	  ...
	  <xsl:apply-templates select="id(@ref)/term" mode="divnum"/>
        </a>
        <xsl:text>"</xsl:text>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:template match="div1|div2|div3|div4|div5|div6" mode="specref">
    <a>
      <xsl:attribute name="href">
        <xsl:call-template name="href.target"/>
      </xsl:attribute>
  ...
  <!-- MJH exception for numbering all tables with captions -->
  <xsl:template
    match="table/caption">
    <caption>
      <xsl:for-each select="@*">
  ...
  <!-- mode: divnum -->
  <xsl:template mode="divnum" match="body//table/caption">
  ...
  <xsl:template mode="divnum" match="back//graphic">
    ...
    <xsl:text>-</xsl:text>
    <xsl:number level="any" from='div1 | inform-div1' count="graphic" format="1"/>
  </xsl:template>

  <xsl:template mode="divnum" match="example/head">
    <xsl:number level="multiple" count="div1" format="1"/>
    <xsl:text>-</xsl:text>
    <xsl:number level="any" from='div1' count="example" format="1"/>
  </xsl:template>
  <xsl:template mode="divnum" match="back//example/head">
    <xsl:number level="multiple" count="div1 | inform-div1" format="A"/>
    <xsl:text>-</xsl:text>
    <xsl:number level="any" from='div1 | inform-div1' count="example" format="1"/>
  </xsl:template>
  ...
  <xsl:template mode="divnum"
    match="front/div1 | front//div2 | front//div3 | front//div4 | front//div5 | front//div6"/>
  ...
  <xsl:template mode="divnum" match="div6">
    <xsl:number level="multiple"
      count="div1 | div2 | div3 | div4 | div5 | div6" format="1.1.1.1.1.1 "/>
  </xsl:template>

  <xsl:template mode="divnum" match="back//div6">
    <xsl:number level="multiple"
      count="div1 | div2 | div3 | div4 | div5 | div6 | inform-div1"
      format="A.1.1.1.1.1 "/>
  </xsl:template>
...
div.exampleHeader { font-weight: bold;
    ...
    <link rel="stylesheet" type="text/css">
      <xsl:attribute name="href">
        ...
        <xsl:choose>
          <xsl:when test="/spec/@role='editors-copy'">W3C-MO</xsl:when>
          <xsl:otherwise>
            <xsl:choose>
	      <!-- Editor's review drafts are a special case. -->
              <xsl:when test="/spec/@w3c-doctype='review'"
  ...
  <link rel="canonical">
      <xsl:attribute name="href">
        <xsl:text>https://www.w3.org/TR/</xsl:text>
        <xsl:value-of select="/spec/header/w3c-canonical" />
      </xsl:attribute>
  </link>
  </xsl:template>
